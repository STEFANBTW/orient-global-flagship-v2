...
function initBookCoverAnimations() {
    ...
    function updateCoverAnimations() {
        ...
        // Calculate progress without query selectors or layout thrashing
    ...
    function applyTransform(element, progress, scrollRange) {
        const scrolledPixels = progress * scrollRange;

        // Translation: Move N pixels for every 1 pixel of scroll
        const translationXRatio = 10;
        const translationYRatio = 0;
        const translateXPixels = scrolledPixels * translationXRatio;
        const translateYPixels = scrolledPixels * translationYRatio;

        // Convert translations to percentages for responsive behavior
        const translateXPercent = Math.min((translateXPixels / screenWidth) * 100, 100);
        const translateYPercent = Math.min((translateYPixels / viewportHeight) * 100, 100);

        // Rotation: Gentler 2D rotation driven by animation progress
        const maxRotation = -200; // degrees anticlockwise
        const angle = progress * maxRotation;

        // Scale: Grow by up to 2% driven by animation progress
        const scale = 1 + (progress * 0.02);

        // Translate left (-translateXPercent), up (-translateYPercent), rotate, and scale
        element.style.transform = `translateX(${-translateXPercent}%) translateY(${-translateYPercent}%) rotate(${angle}deg) scale(${scale})`;

        // Keep cover fully opaque during animation
        element.style.opacity = 1;

        // Hide when completely off screen to allow interaction with content behind
        const maxPercent = Math.max(translateXPercent, translateYPercent);
        if (maxPercent >= 100) {
    ...
    // Event listeners
    window.addEventListener('scroll', updateCoverAnimations, { passive: true });
