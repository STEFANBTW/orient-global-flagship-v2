...
# Using re.sub to replace everything from the first <style> to the last </style>
pattern = re.compile(r'<style>.*?</style>', re.DOTALL)
new_content = re.sub(pattern, new_style, content)

# But wait! re.sub might match <style> to the first </style>, not the last.
...
if start_idx != -1 and end_idx != -1:
    end_idx += len('</style>')
    final_content = content[:start_idx] + new_style + content[end_idx:]
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(final_content)
    print("Replaced all style tags successfully.")
else:
