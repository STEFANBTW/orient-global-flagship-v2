...
| `"purpose": "any maskable"`| Adaptive Icon | App Install | Allows Android to dynamically crop the icon into circles or rounded squares without placing it inside an ugly white box | Icon block | String Enum | Icon optimization|
...
| Line | What | When | Why | Where | How | Which |
|---|---|---|---|---|---|---|
| `self.__WB_MANIFEST` | Injection Target | Build Phase | Rollup replaces this variable with a hardcoded array of hashed UI files (CSS/JS) | `precache...` | Global variable | Pre-cache payload |
...
**Deep Technical Constraint**: Leaflet fetches tiles from OpenStreetMap via standard `<img>` tags, triggering Cross-Origin read requests without explicit CORS permission. The browser network stack fulfills these as **Opaque Responses** (`status 0`).

Opaque responses are a severe security risk in the context of Service Workers because the SW cannot read the headers to determine the file size. Consequently, standard browser quotas calculate *every* Opaque response as requiring **7 Megabytes** of storage space, even if the tile is only 15KB.

Without the `maxEntries: 500` governor and the `purgeOnQuotaError` flag implemented in Section 3, a user rapidly panning the map will instantly trigger an OS-level `QuotaExceededError`, completely bricking the Service Worker and the local database.
...
const updateSW = registerSW({
  onNeedRefresh() {
    // In production, this triggers a custom Toast UI
...
|-----------------|------------|------------------------|------------------|
| **Service Worker fails to register**| Scope Violation | The `sw.js` file was placed inside `/src/` or `/assets/`. A SW can only control URLs deeper than its own file path. | Vite-PWA plugin automatically outputs `sw.js` to the root `dist/` folder. Ensure you are not manually moving it. |
...
| **Vite Dev Server throws SW errors**| Bypass for Network | Service Workers are disabled by default in `npm run dev` to prevent caching issues during active development. | This is expected. To test SW locally, run `npm run build` then `npm run preview`. |
