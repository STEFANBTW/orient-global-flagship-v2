...
| **Levenshtein matrix triggers Garbage Collection pause**| Heap Allocation Loop | `new Array(x)` generated inside the loop causes constant object allocation and destruction. | Move `Uint8Array` allocation outside the loop and swap `v0`/`v1` pointers. |
