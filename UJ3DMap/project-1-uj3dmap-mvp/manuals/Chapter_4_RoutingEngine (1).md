...
2. **Apply Haversine Formula**: Calculate the great-circle distance between two points. This serves as the $h(n)$ heuristic in the A* algorithm.
...
| `[geo[1], geo[0]]` | Coordinate Inversion| Hydration phase | Resolves the core Leaflet vs GeoJSON specification conflict (Lat/Lng vs Lng/Lat) | `hydratePath...` | Array construct | Coordinate swapper |
...
1. **Verify A* Performance Execution Bounds**:
   - Wrap the controller execution in timing telemetry:
     ...
     ```
   - *Validation*: A campus-scale route (1,000 nodes) MUST resolve in `< 15ms`. If it exceeds 50ms, the Min-Heap is failing to bypass `O(N)` sorting.
