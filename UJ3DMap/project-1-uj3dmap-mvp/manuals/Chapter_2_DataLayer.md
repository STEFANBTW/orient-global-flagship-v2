...
| `src/data/validator.js` | Payload Guard | Enforces strict schema validation on incoming JSON payloads to prevent structural crashes. |
...
graph TD
    ...
    AllocMap --> Return[graphMap returned to application]
...
async function initializeDataLayer() {
  try {
    ...
    console.error('[DataLayer] Failure:', error);
  }
}
```
...
|-----------------|------------|------------------------|------------------|
| **`TypeError: NetworkError` on Fetch** | CORS Violation | Browser blocked request because the target server failed to return `Access-Control-Allow-Origin: *` during the `OPTIONS` preflight. | Ensure local dev servers run on identical ports, or add `cors: true` to Vite config. |
...
1. **Verify Network Latency Simulation (Slow 3G)**:
   ...
   - Reload page.
   *Validation*: Fetch should throw `Network latency exceeded 8000ms threshold` error smoothly rather than hanging indefinitely.
...
4. **Verify Garbage Collection Efficacy**:
   ...
   - Take Heap Snapshot.
   *Validation*: Search snapshot for raw `edges` or `nodes` arrays. The arrays should not exist, confirming `rawPayload = null` was successful.
