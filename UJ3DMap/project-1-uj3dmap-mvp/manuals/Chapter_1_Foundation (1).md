...
|-----------|---------|----------------------|
| `.nvmrc` | Node Version Engine Lock | Dictates strict Node.js runtime version to guarantee deterministic V8 engine behaviors. |
...
| `vite.config.js` | Vite Build Engine Options | Configures development server port, Host binding, auto-open behavior, pre-bundling, and Rollup build targets. |
...
| `src/map.js` | Spatial Engine Core Module | Instantiates `L.map` with `preferCanvas`, sets center coordinates/zoom, configures OSM tile layer with CORS, and parses GeoJSON layers. |
...
```mermaid
graph TD
    NVM[.nvmrc Lock] -->|v20.11.1| Runtime[Node.js Runtime Engine]
    Runtime -->|npm ci| Lockfile[package-lock.json Determinism]
    Lockfile -->|Install| NodeModules[node_modules / Vite Engine]
...
1. **Establish Runtime Engine Determinism**: Create `.nvmrc` and `.npmrc` files in the workspace root to lock the Node runtime and package manager resolution behaviors.
...
|---|---|---|---|---|---|---|
| `20.11.1` | Node Engine Version | Environment Init | Guarantees identical V8 garbage collection and compilation behaviors across CI/CD and developer machines | `.nvmrc` | Semantic String | Node LTS Version |
...
| `minify: 'terser'` | AST Minifier Engine | Build phase | Utilizes Terser over ESBuild for superior dead-code elimination (tree shaking) algorithms | `build` block | String Enum | Minification engine |
...
3. **Construct Core Layout**: Define `#map` mount point and inject ESM entry script.
...
| `<noscript>` | Graceful Degradation| Render block | Informs non-JS environments of engine requirements | `body` block | HTML tag | Fallback content |
...
/**
 * Bootstraps the application engine.
...
async function bootstrap() {
  try {
    ...
    // Global Error Boundary for unhandled spatial engine rejections
    window.addEventListener('unhandledrejection', (event) => {
      console.error('[UJ3DMap Engine] Unhandled Promise Rejection:', event.reason);
    });

    Object.freeze(mapInstance); // Prevent external prototype pollution
  } catch (error) {
...
| `Object.freeze(mapInstance)`| Object Immutability | Post-init | Prevents malicious third-party scripts from hijacking Leaflet core methods via prototype pollution | `bootstrap` body| JS Object method | Engine security |
...
1. **Create Map Engine Module (`src/map.js`)**: Import Leaflet and CSS.
...
4. **Instantiate Engine**: Bind coordinate system and view constraints.
...
/**
 * Initializes high-performance Leaflet engine.
...
export async function initMap(containerId = 'map') {
  ...
  // Engine instantiation with hardware acceleration flags
...
| Line | What | When | Why | Where | How | Which |
|---|---|---|---|---|---|---|
| `let currentMapInstance` | Singleton Pointer | Module Load | Tracks engine state across module reloads (HMR) to prevent zombie instances | `map.js` root | Memory reference | State variable |
...
| `zoomSnap: 0.5` | Fractional Zoom | Instantiation | Enables smooth sub-level zooming crucial for high-density campus mapping | `L.map` options | Float value | UX behavior |
...
3. **Attach to Map Engine**: Append the configured layer to the `tilePane`.
...
| Line | What | When | Why | Where | How | Which |
|---|---|---|---|---|---|---|
| `maxNativeZoom: 19` | Tile Extrapolation | Zoom > 19 | Instructs engine to auto-scale zoom 19 tiles when users zoom to level 20, preventing HTTP 404 errors | Tile options | Integer config | Raster scaling |
...
3. **Parse and Validate**: Validate payload structure before passing to the Leaflet engine.
4. **Construct Canvas Markers**: Override default DOM-based markers with high-performance Canvas circles.
5. **Secure Popup Binding**: Construct HTML strings and bind them to layer click events.
...
| **Tiles load outside frame** | `updateWhenZooming: true`| Engine fired HTTP requests for intermediate zoom scales (e.g., z14, z15) while animating to z16. | Set `updateWhenZooming: false` and `updateWhenIdle: true`. |
...
| Vite Engine Config | `vite.config.js` | `vite` | Dev Server `127.0.0.1:3000` | Node.js Main Thread |
